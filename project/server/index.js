...
async function getBaseInfo() {
    let res1 = await axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetSltInfoBySub', {
        "SUB_ID": "5"//项目id 
    }).catch(e => {
        console.log(e);
        res.send(0);
    ...
    // console.log(group_slt_id);

    let res2 = await axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetTerInfo', {
        "IS_ADMIN": "1",
    ...
    let res3 = await axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetTableMeterInfo', {
        "IS_ADMIN": "1",
        "GROUP_SLT_ID": group_slt_id
    }).catch(e => {
        console.log(e);
    ...
    // console.log(m_id_water);
}
getBaseInfo();

// 累计信息
app.get('/getTotalInfo', async (req, res) => {
    await axios.all([
        // 累计交易流水
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetRegionRevenueData', {
            "TID": t_id,  //终端id 用|分隔
        ...
        // 累计交易次数
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetSellDataByNum', {
            "TID": t_id,
            "NUM": "",//条数  为空则查全部
        ...
        res.send([totalMoneyData, totalExchangeData]);
    })).catch(e => {
        console.log(e);
        res.send(0);
    });
...
// 今日信息
app.get('/getTodayEnergy', async (req, res) => {
    await axios.all([
        // 今日用电/用水
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetDayEnergyAmountByTable', {
            "GROUP_MID": m_id_elec
        }),
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetDayEnergyAmountByTable', {
...
// 用能走势图
app.get('/getUseEnergyTrend', async (req, res) => {
    await axios.all([
        // 用电本月
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetDayEPower', {
            "TID": t_id, //
...
// 能耗类型分布
app.get('/getEnergyTypeDistribution', async (req, res) => {
    // 能耗类型分布
    await axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetHomeERegionPowerForFunArea', {
        "TID": t_id, //
        "TIME": date.getFullYear() + '-' + G.prefixInteger((date.getMonth() + 1), 2),//2019，2019-07，2019-07-30
        "TYPE": "M",//Y:年，M:月，D:日
        "MENUS_TYPE": "" //0：电 1：水
...
// 用能排名
app.get('/getUseEnergyRating', async (req, res) => {
    await axios.all([
        // 用能排名 -- 用电
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetUseEnergyDataByRegion', {
        ...
        useEleRatingArr.forEach(data => {
            if (data.L_NAME === '量测1号楼') {
                house1Elec += data.USE_ENERGY;
            }
            if (data.L_NAME === '量测2号楼') {
                house2Elec += data.USE_ENERGY;
            }
            if (data.L_NAME === '量测3号楼') {
...
// 交易趋势分析
app.get('/getExchangeTrend', async (req, res) => {
    await axios.all([
        // 交易趋势分析
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetAllRevenueData', {
            "TID": t_id, //从档案信息中获取
            "TIME": date.getFullYear() + '-' + G.prefixInteger((date.getMonth() + 1), 2),//2019，2019-07，2019-07-30
            "TYPE": "M",//Y:年，M:月，D:日
            "MENUS_TYPE": "0" //0：电 1：水
...
// 其他信息
app.get('/Trailingend/Core.ashx', async (req, res) => {
    ...
    await axios.all([
        // 累计用电金额
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetUseEnergyData', {
            "GROUP_SLT_ID": group_slt_id, //从档案信息中获取
            "TIME": date.getFullYear(),//2019，2019-07，2019-07-30
            "TYPE": "Y",//Y:年，M:月，D:日
        }),
        // 累计用水
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetWUseEnergyData', {
            "GROUP_SLT_ID": group_slt_id, //从档案信息中获取
            "TIME": date.getFullYear(),//2019，2019-07，2019-07-30
            "TYPE": "Y",//Y:年，M:月，D:日
        }),
        ...
        // 项目统计 --- 网关
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetDeviceInfo', {
            "IS_ADMIN": "1",//0:是，1：否 这个输入1 
            "GROUP_SLT_ID": group_slt_id, //档案中获取
        }),
        // 项目统计 --- 用户
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=QuerySubPermissionInfo', {
            "DEFAULT_SLT_ID": "14", // 登录返回的结果中获取
            "IS_SUB_ADMIN": "1" // 默认1
        }),
        // 项目统计 --- 电表
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetHomeInfoByPro', {
            "GROUP_SLT_ID": group_slt_id,
            "MENUS_TYPE": "0"
        }),
        // 项目统计 --- 水表
        ...
        // 抄表成功率 --- 电
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetEReadSuccessRate', {
        ...
        // 实时告警
        axios.post('http://ems.com/Trailingend/Core.ashx?funcname=GetReportInfoByPermission', {
            "GROUP_SLT_ID": group_slt_id
        }),
